...
in
{
  ...
  config = {
    systemd.services = lib.mapAttrs' (name: definition: {
      ...
      value = {
        ...
        script =
          let
            link =
              if definition.targetPackageSubpath == "" then
                definition.targetPackage
              else
                "${definition.targetPackage}/${definition.targetPackageSubpath}";
          in
          ''
