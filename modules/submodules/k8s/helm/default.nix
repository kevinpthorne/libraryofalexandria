...
in
{
  ...
  config =
    let
      ...
      forEachChartModuleBackwards =
        func:
        builtins.map (chart: func chart) (
          lib.lists.reverseList config.libraryofalexandria.helmCharts.charts
        );
              ...
              ${concatCommands (
                forEachChartModule (
                  ...
                  (concatCommands (
                    ...
                    ++ lib.optional (chart._maxRevisions != null) "if ! ${pkgs.kubernetes-helm}/bin/helm status ${chart.name} ${
                      lib.optionalString (chart.namespace != null) "--namespace ${chart.namespace}"
                    } --kubeconfig ${kubeconfig} >/dev/null 2>&1; then REV=0; else REV=$(${pkgs.kubernetes-helm}/bin/helm status ${chart.name} ${
                      lib.optionalString (chart.namespace != null) "--namespace ${chart.namespace}"
                    } --kubeconfig ${kubeconfig} -o json | ${pkgs.jq}/bin/jq -r '.version // 0'); fi; if [ \"$REV\" -lt ${if chart._maxRevisions != null then toString chart._maxRevisions else "1"} ]; then"
                    ++ [
                    ...
                    ++ lib.optional (chart._maxRevisions != null) "else echo \"${chart.name} hit max revisions (${if chart._maxRevisions != null then toString chart._maxRevisions else "1"}), skipping upgrade\"; fi"
                  ))
                )
              )}
            '';
